...
for file in FileList:
    videoPath = os.path.join(DirPath, file)
    ...
    Recoder = cv.VideoWriter(f'result/{file}.mp4', fourcc,
    ...
    while True:
        ...
        if face is not None:
            ...
            cv.circle(image, circleCenter,
                      (int(blinkRatio*4.3)), m.CHOCOLATE, -1)
            ...
            cv.line(image, (int(width-150), 50),
                    (int(width-45), 50), m.WHITE, 30)
            ...
            cv.putText(image, f'Right Eye', (35, 55),
                       m.fonts, 0.6, color[1], 2)
